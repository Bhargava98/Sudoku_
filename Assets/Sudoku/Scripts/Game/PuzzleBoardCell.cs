...
namespace BizzyBeeGames.Sudoku
{
	public class PuzzleBoardCell : ClickableListItem
	{
        ...
        public void SetTextIncorrect()
		{
            ...
            if (GameManager.Instance.stopMistakes == false && GameManager.Instance.isErased == false
                && GameManager.Instance.IsPaused == false && PuzzleBoard.isNotesOn == false && GameManager.Instance.isUndo == false
                )
            {
                GameManager.Instance.currMistakes++;
            }
            if (GameManager.Instance.ActivePuzzleData.mistakes > 0 && GameManager.Instance.currMistakes == GameManager.Instance.ActivePuzzleData.mistakes)
            {
                GameManager.Instance.currMistakes = GameManager.Instance.ActivePuzzleData.mistakes;
                GameManager.Instance.ActivePuzzleData.mistakes = 0;
            }


            //print("Mistakes: "+ GameManager.Instance.ActivePuzzleData.mistakes);

            if (GameManager.Instance.isCellClicked) {
            ...
            GameManager.Instance.setMistakesUI();
        ...
        IEnumerator SetFade()
        {
            Color themeColor = GetThemeColor(selectedPrimaryColor);
                // loop over 1 second backwards
                for (float i = 1; i >= 0; i -= Time.deltaTime*1.5f)
                {
                // set color with i as alpha
                    themeColor.a = i;
                    cellBackground.color = themeColor;
                    yield return null;
                }
        }
        ...
        public bool isEmptyNotes()
        {
            for (int i = 1; i <= noteTextObjs.Count; i++)
            {
                if (i - 1 < noteTextObjs.Count)
                {
                    GameObject textObj = noteTextObjs[i - 1];
