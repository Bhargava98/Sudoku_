		...
		public PuzzleData GetPuzzle()
		{
			...
			for (int i = 0; i < puzzleFiles.Count; i++)
			{
				...
				if (!playedPuzzles.Contains(puzzleIndex))
				{
                    ...
                    //Debug.Log("Puzzle Name :"+ puzzleFiles[puzzleIndex].name);
                    GameManager.Instance.currLevelName = puzzleFiles[puzzleIndex].name;
//#if UNITY_ANDROID || UNITY_IOS
//                    GameAnalytics.LevelStart(puzzleFiles[puzzleIndex].name);
//#endif
