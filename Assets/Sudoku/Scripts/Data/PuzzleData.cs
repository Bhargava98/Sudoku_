...
namespace BizzyBeeGames.Sudoku
{
	public class PuzzleData
	{
        ...
        public bool     is2ndChanceTaken = false;
        ...
        public void RestoreGame()
        {
            ...
            for (int i = 0; i < 9; ++i)
                for (int j = 0; j < 9; ++j)
                {
                    if (cellDatas[i][j].placedNumber != -1)
                    {
                        cellDatas[i][j].placedNumber = -1;
                        cellDatas[i][j].isHint = false;
                    }
                }
            // cellDatas.RemoveAll();
        }
        ...
        public int GetSolvedNumber(int row,int col)
        {
        ...
        public bool SetPlacedNumber(int row, int col, int number)
		{
			...
			{
                if (GetCellType(row, col) == PuzzleData.CellType.Incorrect)
                {
                    RemoveNumbers(row,col);
                    return false;
                }
				return false;
			}
		...
		public Dictionary<string, object> Save()
		{
            ...
            saveData["isGameover"]      = isGameover;
            saveData["puzzleName"]      = puzzleName;
            saveData["is2ndChanceTaken"]= is2ndChanceTaken;  
		...
		private void LoadSave(JSONNode saveData)
		{
            ...
            isGameover = saveData["isGameover"].AsBool;
            puzzleName = saveData["puzzleName"];
            is2ndChanceTaken = saveData["is2ndChanceTaken"].AsBool;
			defaultState	= new List<List<int>>();
			solvedState		= new List<List<int>>();
			cellDatas		= new List<List<CellData>>();
			undoStack		= new List<List<UndoItem>>();
            mistakes        = saveData["mistakes"].AsInt;
