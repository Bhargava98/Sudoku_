...
namespace BizzyBeeGames
{
	public class ToggleSlider : UIMonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
	{
		...
		private Color			handleOnColor		= Color.white;
		private Color			handleOffColor		= Color.white;
		...
		private void Awake()
		{
            handleOnColor = GetThemeColor("popup_foreground");
            handleOffColor = GetThemeColor("handle_off");
		...
		{
            SetToggle(!IsOn, true);
            if (IsOn && gameObject.transform.parent.name.Contains("Vibration")) Handheld.Vibrate();
            else if (IsOn) SoundManager.Instance.Play("btn-click");
        }
